...
public class CopySaplingBlock extends BlockSapling {
            ...
            for (int dz = -1; dz <= 1; dz++) {
                for (int dy = 0; dy <= 2; dy++) {
                    ...
                    if (!isInner && world.isAirBlock(posX, posY, posZ)) {
                        world.setBlock(posX, posY, posZ, below, below.getMetaFromState(below.getDefaultState()), 3);
                    }
                }
            }
        }
    ...
    public static class SaplingGrowHandler {
        @SubscribeEvent
        public void onBonemeal(BlockEvent.BonemealEvent event) {
            if (event.block instanceof CopySaplingBlock) {
                ...
                if (!event.world.isRemote) {
                    ...
                    event.setResult(net.minecraftforge.event.world.BlockEvent.Result.ALLOW);
                }
            }
        }
    }
